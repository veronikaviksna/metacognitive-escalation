# convfinqa_slm_1000.py — ConvFinQA SLM Baseline: Qwen 3B, 1000 questions
# Inference: 3 traces with majority voting (self-consistency)
#
# Data format: train.json from https://github.com/czyssrs/ConvFinQA
# Table linearisation: table_ori with official ConvFinQA format
# Context: pre_text + table_ori + post_text + prior Q/A history (no leakage)
...
OUT_PATH       = BASE_DIR / "convfinqa_slm_results.csv"
UIDS_PATH      = BASE_DIR / "convfinqa_sample_uids.json"
CHECKPOINT     = BASE_DIR / "convfinqa_slm_checkpoint.json"
...
if uid_file:
    ...
    # Sort to guarantee identical order with hierarchical and LLM runs
...
acc = sum(1 for r in results if r["correct"]) / N
print(f"\n{'='*60}")
print(f"ConvFinQA SLM BASELINE — SUMMARY")
print(f"{'='*60}")
print(f"  Model   : {MODEL}")
print(f"  N       : {N}")
