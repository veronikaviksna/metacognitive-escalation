# finqa_hierarch_1000_s3-3_s4-soft.py — FinQA Metacognitive Hierarchical Pipeline
...
#   SOFT      : S2=False, S3=2, S4<0.5                      → raise MIN_S5
...
# ── CONTROL THRESHOLDS ────────────────────────────────────────────────────
MIN_S5            = 0.1
S3_HARD_THRESHOLD = 3     # hard flag: S3 level >= 3
DELTA_S2          = 0.2   # soft: S2=False raises threshold
DELTA_S3_SOFT     = 0.1   # soft: S3==2 (moderate complexity) raises threshold
DELTA_S4          = 0.1   # soft: S4<0.5 (low reasoning quality) raises threshold
...
def slm_control(s2, s3_level, s3_hard, s4, s5, s6_ok, majority, s1_task, s_operand):
    ...
    #   S2=False : data sufficiency uncertain     → +0.2
    #   S3==2    : moderate complexity            → +0.1
    #   S4<0.5   : low reasoning process quality  → +0.1
    ...
    if s3_level == 2:
        ...
        soft_adj.append(f"S3=2 (+{DELTA_S3_SOFT})")
    if s4 < 0.5:
