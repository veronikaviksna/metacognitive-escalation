...
# Runs ONLY the large model (Qwen2.5-14B-Instruct) on the 1000 shared UIDs.
# Loads finqa_sample_uids.json written by finqa_hierarch_1000.py.
# No escalation — every answer comes from the LLM majority vote.
# Output: finqa_llm_results.csv
...
BASE_DIR   = Path(".")
FINQA_JSON = Path("train.json")
OUT_PATH        = BASE_DIR / "finqa_llm_results.csv"
CHECKPOINT_PATH = BASE_DIR / "finqa_llm_checkpoint.json"
...
if uid_file:
    ...
    # Fallback: sample fresh and save
...
def generate(model, tok, prompt, max_new_tokens=450, do_sample=False, temperature=0.7):
...
# ── PROMPT ────────────────────────────────────────────────────────────────
def p_llm_trace(question, context, is_bool=False):
...
# MAIN LOOP
# ══════════════════════════════════════════════════════════════════════════
model_llm, tok_llm = load_model(LARGE_MODEL)
...
for q_num, row in sample.iloc[_start_idx:].iterrows():
    ...
    for i in range(N_TRACES):
        t_raw = generate(model_llm, tok_llm,
                         p_llm_trace(question, ctx_full, is_bool),
                         max_new_tokens=450, do_sample=(i > 0), temperature=0.7)
    ...
    results.append({
        ...
        "n_traces": N_TRACES, "version": "finqa_llm_1000",
    })
...
unload_model(model_llm); del tok_llm
...
acc = sum(1 for r in results if r["correct"]) / N
print(f"\n{'='*50}")
print(f"LLM-ONLY (FinQA)  N={N}  Accuracy={acc:.1%}")
print(f"{'='*50}")
