# tatqa2_slm_1000.py — TAT-QA SLM Baseline: Qwen 3B, 1000 questions
...
OUT_PATH   = os.path.join(BASE_DIR, "tatqa_slm_results.csv")
UIDS_PATH  = os.path.join(BASE_DIR, "tatqa_sample_uids.json")
CHECKPOINT = os.path.join(BASE_DIR, "tatqa_slm_checkpoint.json")
...
def p_slm(question, context):
    ...
    return (
        ...
        "Step 2 - If calculation is needed, show arithmetic step by step.\n"
...
def generate_one(model, tok, prompt, max_new_tokens=350, do_sample=True):
...
for i, row in enumerate(todo):
    ...
    answer, raws, trace_answers = generate_with_voting(mdl, tok, p_slm(question, context))
...
# ── SUMMARY ───────────────────────────────────────────────────────────────
print(f"""
TAT-QA SLM ACCURACY ({n_total} questions)
  Correct : {correct}/{n_total} = {correct/n_total:.1%}
  Model   : Qwen 3B, {N_TRACES} traces + majority voting
""")
